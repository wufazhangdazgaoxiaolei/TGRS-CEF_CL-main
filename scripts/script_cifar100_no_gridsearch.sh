...
do
  if [ "$3" = "base" ]; then
        ...
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                ...
                --nepochs 90 --batch-size 32 --results-path "$PROJECT_DIR/imagenet_subset/$5base_$6tasks" \
                --approach $1 --gpu $2 --lr 0.1 --lr-factor 10 --momentum 0.9 \
                --weight-decay 1e-4 \
                --nc-first-task $5 --schedule_step 30 60 \
                --num-exemplars-per-class 20 --exemplar-selection herding
    elif [ "$3" = "NWPU-RESISC45" ]; then
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                --datasets "$3_$4" --num-tasks $6 --network resnet34 --seed $SEED \
                --nepochs 100 --batch-size 32 --results-path "$PROJECT_DIR/NWPU-RESISC45/$5base_$6tasks" \
                --approach $1 --gpu $2 --lr 0.5 --lr-factor 10 --momentum 0.9 \
                --weight-decay 1e-4 \
                --nc-first-task $5 --schedule_step 30 60 \
                --num-exemplars-per-class 20 --exemplar-selection herding
    elif [ "$3" = "AID" ]; then
        PYTHONPATH=$SRC_DIR python3 -u $SRC_DIR/main_incremental.py --exp-name no_gs_fixd_${SEED} \
                --datasets "$3_$4" --num-tasks $6 --network resnet34 --seed $SEED \
                --nepochs 100 --batch-size 32 --results-path "$PROJECT_DIR/AID/$5base_$6tasks" \
                --approach $1 --gpu $2 --lr 0.5 --lr-factor 10 --momentum 0.9 \
                --weight-decay 1e-4 \
                --nc-first-task $5 --schedule_step 30 60 \
                --num-exemplars-per-class 20 --exemplar-selection herding
