...
from .cmo import rand_bbox, getbox_new
...
class Appr(Inc_Learning_Appr):
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        # CMO
        # get each task cls num
        cls_num_dict = {}
        for _, label in trn_loader.dataset:
            cls_num_dict[label] = cls_num_dict.get(label, 0) + 1
        print('cls num dict:')
        print(cls_num_dict)
        ...
        samples_weight = np.array(
            [cls_weight[t] for _, t in trn_loader.dataset])
    ...
    def train_epoch(self, t, trn_loader, weighted_train_loader):
        ...
        for images, targets in trn_loader:
            ...
            if r < self.mixup_prob:  # Resize mix
                ...
                loss = self.criterion(t, outputs, targets, features, fmaps, ref_features, ref_fmaps) * lam + \
                       self.criterion(t, outputs, targets, features, fmaps, ref_features, ref_fmaps) * (
                               1. - lam)
            else:
                outputs, features_ = self.model(images, return_features=True)
                fmaps = features_['fmaps']
