...
class Appr(Inc_Learning_Appr):
    ...
    def train_loop(self, t, trn_loader, val_loader, args, tst_loader):
        ...
        if t >= 0:
            ...
            # 对协方差矩阵进行修正，确保其是正定的
            for i in range(cov_cls_ms.shape[0]):
                cov = cov_cls_ms[i]
                eigenvalues, eigenvectors = torch.linalg.eigh(cov, UPLO='U')  # 使用torch.linalg.eigh代替torch.symeig
                min_eigenvalue = torch.min(eigenvalues)
                if min_eigenvalue < 0:
                    cov -= 10 * min_eigenvalue * torch.eye(cov.shape[0])
