...
class Inc_Learning_Appr:
    ...
    def test(self, t, val_loader, return_pred_targets=False):
        predictions = []
        targets_all = []
        """Contains the evaluation code"""
        with torch.no_grad():
            ...
            label = []
            for images, targets in val_loader:
                feat = self.model.model(images.to(self.device)).cpu().numpy()
        ...
        last_five_labels = np.unique(test_labels)[-4:]
        for label in last_five_labels:
            indices = test_labels == label
            plt.scatter(tsne_results[indices, 0], tsne_results[indices, 1], label=str(label), s=70)
        ax = plt.gca()
        ...
        filename = f'WACV_tsne_ltio{timestamp}.png'
        plt.savefig(filename)
        return total_loss / total_num, total_acc_taw / total_num, total_acc_tag / total_num

    '''
    def eval_test(self, t, val_loader, num_per_class):
        """Contains the evaluation code"""
        many_shot = num_per_class > 100
        medium_shot = (num_per_class <= 100) & (num_per_class >= 20)
        few_shot = num_per_class < 20
        with torch.no_grad():
            total_many_shot_tag, total_medium_shot_tag, total_few_shot_tag = 0, 0, 0
            total_many_shot_num, total_medium_shot_num, total_few_shot_num = 0, 0, 0
            self.model.eval()
            for images, targets in val_loader:
                many_shot_index = []
                medium_shot_index = []
                ...
                for i, target in enumerate(targets):
                    if many_shot[target]:
                        many_shot_index.append(i)
                    elif medium_shot[target]:
                        medium_shot_index.append(i)
                    elif few_shot[target]:
                        few_shot_index.append(i)
                # Forward current model
                outputs = self.model(images.to(self.device))
                loss = self.criterion(t, outputs, targets.to(self.device))
                hits_taw, hits_tag = self.calculate_metrics(outputs, targets)
                # Log
                for i, hit in enumerate(hits_tag):
                    if i in many_shot_index and hit == 1:
                        total_many_shot_tag += 1
                    elif i in medium_shot_index and hit == 1:
                        total_medium_shot_tag += 1
                    elif i in few_shot_index and hit == 1:
                        total_few_shot_tag += 1
                total_many_shot_num += len(many_shot_index)
                total_medium_shot_num += len(medium_shot_index)
                total_few_shot_num += len(few_shot_index)
            if total_many_shot_num != 0:
                total_many_shot_tag /= total_many_shot_num
            if total_medium_shot_num != 0:
                total_medium_shot_tag /= total_medium_shot_num
            if total_few_shot_num != 0:
                total_few_shot_tag /= total_few_shot_num
        return total_many_shot_tag, total_medium_shot_tag, total_few_shot_tag
    '''
