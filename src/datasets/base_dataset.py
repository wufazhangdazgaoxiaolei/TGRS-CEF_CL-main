...
def get_data(path, num_tasks, nc_first_task, validation, shuffle_classes, class_order=None, lt=False, ltio=False):
    ...
    if validation > 0.0:
        for tt in data.keys():
            for cc in range(data[tt]['ncla']):
                ...
                if int(np.round(len(cls_idx) * validation)) < 1:
                    rnd_img = [random.choice(cls_idx)]
                else:
    ...
    for tt in data.keys():
        train_count = len(data[tt]['trn']['x'])
        val_count = len(data[tt]['val']['x'])
        print(f"任务{tt}：训练集样本数量={train_count}, 验证集样本数量={val_count}")
    # other
    n = 0
    for t in data.keys():
