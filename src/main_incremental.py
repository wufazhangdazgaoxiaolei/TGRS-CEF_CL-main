...
def main(argv=None):
    ...
    many_shot_taw = np.zeros((max_task))
    medium_shot_taw = np.zeros((max_task))
    few_shot_taw = np.zeros((max_task))
    for t, (_, ncla) in enumerate(taskcla):
        ...
        '''
        # test many medium few
        many_num, medium_num, few_num = 0, 0, 0
        for u in range(t + 1):
            many_taw, medium_taw, few_taw = appr.eval_test(u, tst_loader[u], num_per_class)
            many_shot_taw[t] += many_taw
            medium_shot_taw[t] += medium_taw
            few_shot_taw[t] += few_taw
            if many_taw != 0:
                many_num += 1
            if medium_taw != 0:
                medium_num += 1
            if few_taw != 0:
                few_num += 1
        if many_num != 0:
            many_shot_taw[t] /= many_num
        if medium_num != 0:
            medium_shot_taw[t] /= medium_num
        if few_num != 0:
            few_shot_taw[t] /= few_num
        print('>>> Many shot TAw acc={:5.1f}%, Medium shot TAw acc={:5.1f}%, '
              'Few shot TAw acc={:5.1f}%,<<<'.format(100 * many_shot_taw[t], 100 * medium_shot_taw[t],
                                                     100 * few_shot_taw[t]))
        '''
        # Save
    ...
    '''
    many_shot_non_zero_count = sum(1 for i in many_shot_taw if i != 0)  # 统计many_shot_taw中不为零的元素个数
    avg_many_shot_taw = sum(many_shot_taw) / many_shot_non_zero_count  # 计算平均值，除以不为零的元素个数
    print('>>> Many shot TAw acc={:5.1f}% <<<'.format(100 * avg_many_shot_taw))

    medium_shot_non_zero_count = sum(1 for i in medium_shot_taw if i != 0)  # 统计medium_shot_taw中不为零的元素个数
    avg_medium_shot_taw = sum(medium_shot_taw) / medium_shot_non_zero_count  # 计算平均值，除以不为零的元素个数
    print('>>> Medium shot TAw acc={:5.1f}% <<<'.format(100 * avg_medium_shot_taw))

    few_shot_non_zero_count = sum(1 for i in few_shot_taw if i != 0)  # 统计few_shot_taw中不为零的元素个数
    avg_few_shot_taw = sum(few_shot_taw) / few_shot_non_zero_count  # 计算平均值，除以不为零的元素个数
    print('>>> Few shot TAw acc={:5.1f}% <<<'.format(100 * avg_few_shot_taw))
    print('*' * 108)
    '''
    elapsed_time = time.time() - tstart
    hours = elapsed_time / (60 * 60)
    minutes = elapsed_time / 60
    print('[Elapsed time = {:.2f} h ({:.0f} min)]'.format(hours, minutes))
    print('Done!')
